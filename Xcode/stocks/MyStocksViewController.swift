...
class MyStocksViewController: UIViewController {
    ...
    override func viewDidLoad() {
            super.viewDidLoad()
            navigationController?.navigationBar.titleTextAttributes = [.foregroundColor: UIColor.white] // Ensures title color is white
...
private extension MyStocksViewController {
    ...
    func updateNavBar(_ isEditing: Bool = false) {
        if isEditing {
            ...
            navigationItem.rightBarButtonItems = dataSource.first?.items?.count ?? 0 == 0 ? [addButton] : [editButton, addButton]
        }
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        ...
        func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            if let item = s.items?[indexPath.row] {
                ...
                cell.selectionStyle = .blue // Make sure the selection does not make the text disappear
            }
